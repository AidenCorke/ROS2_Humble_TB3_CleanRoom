...
# --- Imports ---
#=======================================
from clean_room.planner.mapping.waypoint_mesher import WaypointMesh
from clean_room.planner.planning.tsp_solver_greedy import GreedyTSP
import numpy as np
...
class RoomPathPlanner:
    ...
    def compute_free_mask(self):
        ...
        self.inflated = self.mp.inflate_obstacles(self.room, self.inflation_radius)  # Create a mask of inflated obstacles 
        self.free_mask = np.logical_and(free, ~self.inflated)                        # Remove intersecting cells from the free mask and the inflated mask
    ...
    def compute_full_path(self, start_index=0):
        ...
        path = [self.waypoints[i] for i in order]

        #visualize_room_and_path(self.room,self.waypoints,path)
        #visualize_inflation(self.room,self.inflated)
